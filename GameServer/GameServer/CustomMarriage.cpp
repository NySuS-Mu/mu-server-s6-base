...
void CCustomMarriage::ReadCustomMarriageInfo(char* section,char* path) // OK
{
	...
	this->m_CustomMarriageNPC = GetPrivateProfileInt(section,"CustomMarriageNPCShopID",0,path);
...
void CCustomMarriage::CommandPropuse(LPOBJ lpObj,char* arg) // OK
{
	...
	if(lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(882));
		return;
	}

	if(lpObj->Index == lpTarget->Index)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(550));
		return;
	}

	if(lpObj->Interface.use != 0 || lpTarget->Interface.use != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(551));
		return;
	}

	if(lpObj->Married != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(552),lpObj->MarryName);
		return;
	}

	if(lpTarget->Married != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(553),lpTarget->Name);
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(556),this->m_WeddingItemCount,gItemManager.GetItemName(this->m_WeddingItemIndex,this->m_WeddingItemLevel));
		return;
	}
	...
	gNotice.GCNoticeSend(lpTarget->Index,1,0,0,0,0,0,gMessage.GetMessage(558),lpObj->Name);
...
void CCustomMarriage::CommandAccept(LPOBJ lpObj) // OK
{
	...
	for(int n=0;n < 10;n++)
	{
		GCServerCommandSend(lpObj->Index, (lpObj->X + (((GetLargeRand() % 5) * 2) - 4)), (lpObj->Y + (((GetLargeRand() % 5) * 2) - 4)), 0);
	}

	gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(561),lpTarget->Name);
	
	gNotice.GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,gMessage.GetMessage(561),lpObj->Name);

	if(this->m_CustomMarriageAnnounce != 0)
	{
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(562),lpObj->Name,lpTarget->Name);
	}
...
void CCustomMarriage::CommandDivorce(LPOBJ lpObj) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(564),this->m_DivorseItemCount,gItemManager.GetItemName(this->m_DivorseItemIndex,this->m_DivorseItemLevel));
		return;
	}
	...
	else
	{
		...
		gNotice.GCNoticeSend(lpTarget->Index,0,0,0,0,0,0,gMessage.GetMessage(565));
	}
	...
	gNotice.GCNoticeSend(lpObj->Index,0,0,0,0,0,0,gMessage.GetMessage(565));
}
...
void CCustomMarriage::CommandTrackMarry(LPOBJ lpObj) // OK
{
	...
	if(lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(566));
		return;
	}

	if(lpObj->Interface.use != 0 || CA_MAP_RANGE(lpTarget->Map) != 0 || KALIMA_MAP_RANGE(lpTarget->Map) != 0 || DS_MAP_RANGE(lpTarget->Map) != 0 || BC_MAP_RANGE(lpTarget->Map) != 0 || CC_MAP_RANGE(lpTarget->Map) != 0 || IT_MAP_RANGE(lpTarget->Map) != 0 || DA_MAP_RANGE(lpTarget->Map) != 0 || DG_MAP_RANGE(lpTarget->Map) != 0 || IG_MAP_RANGE(lpTarget->Map) != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(567));
		return;
	}

	if((lpTarget->Map == MAP_ICARUS || lpTarget->Map == MAP_KANTURU3) && (lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[8].m_Index != GET_ITEM(13,3) && lpObj->Inventory[8].m_Index != GET_ITEM(13,37))) // Dinorant,Fenrir
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(567));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(568));
		return;
	}
