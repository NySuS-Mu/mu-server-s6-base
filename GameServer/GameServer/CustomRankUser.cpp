		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					this->m_Data[index].m_Ruud = lpMemScript->GetAsNumber();
...
void CCustomRankUser::GCReqRankLevelUser(int aIndex, int aTargetIndex)
{
	...
	memcpy(pMsg.MarryName, gObj[aTargetIndex].MarryName, sizeof(pMsg.MarryName));
	// ---
...
void CCustomRankUser::CheckUpdate(LPOBJ lpObj)
{
	...
	for(int i = 0; i < this->m_count; i++)
	{
		if(this->m_Data[i].m_iResetMin == Valor)
		{
			...
			//-- Reward
			GDSetCoinSend(lpObj->Index, this->m_Data[i].m_Coin1, this->m_Data[i].m_Coin2, this->m_Data[i].m_Coin3, this->m_Data[i].m_Ruud,"RankUser");
