...
void CSkillManager::CGSkillTeleportAllyRecv(PMSG_SKILL_TELEPORT_ALLY_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE && lpObj->X > 54 && lpObj->X < 127 && lpObj->Y > 111 && lpObj->Y < 118)
	{
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE && lpObj->X > 67 && lpObj->X < 118 && lpObj->Y > 158 && lpObj->Y < 164)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You cannot use this skill near this location.");
		return;
	}
	...
	if(lpObj->Map == MAP_CASTLE_SIEGE && lpObj->X > 79 && lpObj->X < 105 && lpObj->Y > 201 && lpObj->Y < 207)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"You cannot use this skill near this location.");
		return;
	}
