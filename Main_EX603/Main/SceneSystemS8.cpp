...
void LogoMUm801()
{
  if ( SceneFlag == 2 )
  {
      ...
      *(float*)0xE8CB18 = *(float*)0xE8CB18 + 0.01999999955296516;
      ...
      RenderBitmap(531019, 217.60001 + Fix_DisplayWinExt, 25.0, 204.8, 102.4, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
      EnableAlphaTest(1);
	  glColor4f(*(float*)0xE8CB18, *(float*)0xE8CB18, *(float*)0xE8CB18, *(float*)0xE8CB18);
      RenderBitmap(531018, 217.60001 + Fix_DisplayWinExt, 25.0, 204.8, 102.4, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
	  //
...
void CScene803()
{
	SetDword((LPVOID)0x004D75F0, 113);//Select Server
...
void SScene803()
{
	...
	{
		SetDword((LPVOID)0x004D6C2C, 112);//Select Character
	...
	{
		SetDword((LPVOID)0x004D6C2C, 111);//Select Character s13
		SetRange((LPVOID)0x0063D1CE, 5, ASM::NOP);
