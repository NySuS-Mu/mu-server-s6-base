...
#define pDrawBarForm3			((void(__cdecl*)(float PosX, float PosY, float Width, float Height, float Arg5, int Arg6)) 0x6378A0)
...
void PetMuunHP(signed int x, signed int y, int nameIndex, signed int hp, signed int hpmax, char a9)
{
	...
	if( MinimizarPet == 0 )
	{
		...
		pDrawBarForm3(4, y + 38, Return2X(TransForX( v15 )), ReturnY(TransForY( 4 )), 0, 0);
		pGLSwitch();
		pSetBlend(0);
...
void CharacterInfoWindow(int a1)
{
	...
	/*if (pCheckWindow(pWindowThis(), Inventory))
	{
		...
		{
			*(signed int *)(This + 16) = 83;
		}
		else
		{
			*(signed int *)(This + 16) = 283;
		}
	}
	else
	{
		if (g_StatsAdvance.m_Enable)
		{
			*(signed int *)(This + 16) = 272;
		}
		else
		{
			*(signed int *)(This + 16) = 432;
		}
	}?*
	...
	*(signed int *)(This + 16) = 450 + JCResto + Fix_DisplayWinExt;

	widht = 193;
	pDrawGUI(61532, x, y, 190, 422.0);
	...
	pDrawText(pTextThis(), x + 5, y + 60, v10, 50, 0, (LPINT)3, 0);
	...
	//-- Button Advanced Status
	if( CheckButtonPressed(x + 50, y + 53, 43, 17) )
	{
		
		if (pCheckWindow(pWindowThis(), Inventory))
		{
			g_StatsAdvance.m_Enable = true;
			pDrawGUI(61538, x - 395, y, 213.0f, 422.0f);
			PrintAdvanceStatus(x - 370, y);
		}
		else
		{
			g_StatsAdvance.m_Enable = true;
			pDrawGUI(61538, x - 210, y, 213.0f, 422.0f);
			PrintAdvanceStatus(x - 188, y);
		}
		
		RenderBitmap(61535, x + 50, y + 53, 43, 17, 0.0, 0.2235384732, 0.5035384717, 0.2215384692, 1, 1, 0);
	}
	else
	{
		g_StatsAdvance.m_Enable = false;
		RenderBitmap(61535, x + 50, y + 53, 43, 17, 0.0, 0.0, 0.5035384717, 0.2215384692, 1, 1, 0);
	}
	pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, pGetTextLine(pTextLineThis, 3841));// Advanced Status
	pDrawText(pTextThis(), x + 50, y + 56, v10, 43, 0, (LPINT)3, 0);

	//-- Button Pet
	sub_779410((char *)(This + 1228), x + 95, y + 53, 43, 17);
	if( CheckButtonPressed(x + 95, y + 53, 43, 17) || pCheckWindow(pWindowThis(), PetInfo))
	{
		RenderBitmap(61535, x + 95, y + 53, 43, 17, 0.0, 0.2235384732, 0.5035384717, 0.2215384692, 1, 1, 0);
	}
	else
	{
		RenderBitmap(61535, x + 95, y + 53, 43, 17, 0.0, 0.0, 0.5035384717, 0.2215384692, 1, 1, 0);
	}//-- over 0.4460069239
	pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, pGetTextLine(pTextLineThis, 1217));// Pet
	pDrawText(pTextThis(), x + 95, y + 56, v10, 43, 0, (LPINT)3, 0);

	//-- Button MasterSkill
	sub_779410((char *)(This + 1400), x + 140, y + 53, 43, 17);
	if( CheckButtonPressed(x + 140, y + 53, 43, 17) )
	{
		RenderBitmap(61535, x + 140, y + 53, 43, 17, 0.0, 0.2235384732, 0.5035384717, 0.2215384692, 1, 1, 0);
	}
	else
	{
		RenderBitmap(61535, x + 140, y + 53, 43, 17, 0.0, 0.0, 0.5035384717, 0.2215384692, 1, 1, 0);
	}//-- Click 0.2235384732
	pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, pGetTextLine(pTextLineThis, 1300));
	pDrawText(pTextThis(), x + 140, y + 56, v10, 43, 0, (LPINT)3, 0);
	...
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, "(%d%%)", v65);
	pDrawText(pTextThis(), x + 130, y + 143, v10, 35, 0, (LPINT)4, 0);
	//-- Fruis Decrease
	pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
	ZeroMemory(v10, sizeof(v10));
	...
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, "(%d%%)", v65);
	pDrawText(pTextThis(), x + 130, y + 154, v10, 35, 0, (LPINT)4, 0);
...
//=============================
	...
	int Width = TransForX( 187 );
	int Heigth = TransForY( 429 );

	if (pCheckWindow(pWindowThis(), Character))
	{
		*(signed int*)(This + 36) = Return2X(TransFor2X(MAX_WIN_WIDTH) - (Width * 2)-3);
	}
	else
	{
		*(signed int*)(This + 36) = Return2X(TransFor2X(MAX_WIN_WIDTH) - Width);
	}
...
//	gPostInterface.DrawItemIMG(Return2X(x + TransForX(135)), ReturnY(TransForY(164)), Return2X(TransForX(35)), ReturnY(TransForY(40)), ITEM(14, 173), 0, 0, 0, 0);
	...
	*(DWORD *)(MainSlot + 40) = Return2X(x + (Width / 2) - TransFor2X(t / 2));
	...
	pSetTextColor(pTextThis(), 0xFFu, 0xFFu, 0xFFu, 0xFFu);
	ZeroMemory(v10, sizeof(v10));
	wsprintf(v10, pGetTextLine(pTextLineThis, 3451));
...
void WindowsInfoInventoryExt(int a1)
{
	...
	int Width = TransForX( 180 );
	int Heigth = TransForY( 429 );

	if (pCheckWindow(pWindowThis(), Character))
	{
		*(signed int*)(This + 20) = Return2X(TransFor2X(MAX_WIN_WIDTH) - (Width * 3));
	}
	else
	{
		*(signed int*)(This + 20) = Return2X(TransFor2X(MAX_WIN_WIDTH) - (Width * 2));
	}
...
__declspec (naked) void RenderRect()
{
	...
	*(DWORD *)(main1 + 48) = *(DWORD *)(main1 + 56) * RetangleInventory( 17 ); // W
	*(DWORD *)(main1 + 52) = (int)*(DWORD *)(main1 + 60) * 17; // H
	
	*(float *)(main1 + 88) = 0.32f;
	*(float *)(main1 + 92) = 0.201;
	*(float *)(main1 + 96) = 0.5f;
...
void ItemSlotEmpty(DWORD id, float x, float y, float w, float h)
{
	...
	i = (y - *(DWORD *)(main1 + 44)) / 20;
	j = (x - *(DWORD *)(main1 + 40)) / 20;

	v5 = (double)(ReturnY(TransForY(17)) * i + *(DWORD *)(main1 + 44));//-- y
	v6 = (double)(Return2X(TransForX(17)) * j + *(DWORD *)(main1 + 40));//-- x

	id = (id == 31339) ? 61527 : id;

	pDrawGUI(id, v6, v5, Return2X(TransForX(17)), ReturnY(TransForY(17)));
}
...
void WindowsInfoWareHouse(int a1)
{
	...
	{
		*(signed int*)(This + 16) = 90 + Fix_DisplayWinExt + JCResto;
	}
	else
	{
		*(signed int*)(This + 16) = 270 + Fix_DisplayWinExt + JCResto;
	}
	...
	if ( *(DWORD *)(This + 540) )
	{
		...
		*(DWORD *)((*(DWORD *)(This + 540)) + 44) = 73;
		if (pCheckWindow(pWindowThis(), ExpandInventory))
		{
			*(DWORD*)((*(DWORD*)(This + 540)) + 40) = 120 + Fix_DisplayWinExt + JCResto;
		}
		else
		{
			*(DWORD*)((*(DWORD*)(This + 540)) + 40) = 300 + Fix_DisplayWinExt + JCResto;
		}
	}
...
void WindowsInfoShop(int a1)
{
	...
	if (pCheckWindow(pWindowThis(), Inventory))
	{
		*(signed int*)(This + 20) = 265 + Fix_DisplayWinExt + JCResto;
	}
...
char WindowInfoCommand(int a1)
{
	...
	*(signed int *)(This + 16) = Return2X(TransFor2X(MAX_WIN_WIDTH) - Width);
	x = TransFor2X((double)*(signed int *)(This + 16));
	y = TransForY((double)*(signed int *)(This + 20));
...
int WindowNPCJulia(int a1)
{
	...
	*(signed int *)(This + 24) = Return2X(TransFor2X(MAX_WIN_WIDTH) - Width);
...
void MuHelperInfo(DWORD id, float x, float y, float w, float h)
{
	...
	*(signed int *)(v170 + 200) = Return2X(TransFor2X(MAX_WIN_WIDTH) - w);
...
void RenderPane_Esc(DWORD id, float x, float y, float w, float h)
{
	...
	h = nInterface.ESCAPE_PANEL->Height;
	w = nInterface.ESCAPE_PANEL->Width;
	x = nInterface.ESCAPE_PANEL->X;
	y = nInterface.ESCAPE_PANEL->Y;

	nInterface.DrawWindowBGPanelEX700(nInterface.ESCAPE_PANEL, "Game System", 175);
	//pSetFont(pTextThis(), (int)pFontBold);
	//pSetBackgroundTextColor(pTextThis(), 0, 0, 0, 0);
...
{
	h = nInterface.MIX_PANEL->Height;
	w = nInterface.MIX_PANEL->Width;
	x = nInterface.MIX_PANEL->X;
	y = nInterface.MIX_PANEL->Y;

	nInterface.DrawWindowBGPanelEX700(nInterface.MIX_PANEL, "", 176);
...
//-- CharacterInfo
	...
	SetByte((0x00843AF8 + 1), 0x96);  //Color R
	SetByte((0x00843AF3 + 1), 0xFF);  //Color G
	SetByte((0x00843AEE + 1), 0xF0);  //Color B
	SetByte((0x00843B0F + 1), 190);  //Position X
	SetByte((0x00843B25 + 2), 10);  //Position Y
