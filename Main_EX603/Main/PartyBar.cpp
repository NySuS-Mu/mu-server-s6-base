...
char PrintfPartyBar(int a1)
{
	...
	if (*(BYTE *)(v43 + 917))
	{
		...
		for (i = 0; i < QTPary.PartyMemberCount; ++i)
		{
			...
			//V? icon buff trên thanh bar
			gPartyBuffs.DrawPartyBuffs((double)x, (double)y + 21.0, QTPary.JCPartyDB[i].Name);

			if (strcmp(QTPary.JCPartyDB[i].Name, (char*)(*(DWORD*)0x7BC4F04 + 56)))
			{
				if (gElemental.gDrawButton((int)x - 20, (int)y + 4, 60, 12, "Move", 20))
				{
					gBAiFindPath.TimDuongDi(*(DWORD*)(Hero + 172), *(DWORD*)(Hero + 176), (int)QTPary.JCPartyDB[i].PosX, (int)QTPary.JCPartyDB[i].PosY);
				}
			}
		}
		pGLSwitch();
...
bool CheckButtonPressed_(int x, int y, int w, int h) 
{

	return pCheckMouseOver(x, y, w, h) && *(BYTE*)0x8793386;
}

char PrintfPartyBarEx700(int a1)
{
	char result; // al@2
	float y; // ST08_4@5
	float x; // ST04_4@5
	...
	if (*(BYTE*)(v43 + 917))
	{
		...
		for (i = 0; i < QTPary.PartyMemberCount; ++i)
		{
			...
			if (nInterface.IsWorkZone(x - 8, (double)(y + Salto), Return2X(TransForX(87)), ReturnY(TransForY(30))))
			{
				RenderBitmap(81531, x - 8, (double)(y + Salto), Return2X(TransForX(87)), ReturnY(TransForY(30)), 0.0, 0.0, 1.0, 1.0, 1, 1, 0);
			}
			else
			{
				RenderBitmap(81529, x - 8, (double)(y + Salto), Return2X(TransForX(87)), ReturnY(TransForY(30)), 0.0, 0.0, 1.0, 1.0, 1, 1, 0);
			}

			if (i)
			{
				SetTextColorByHDC(pTextThis(), pMakeColor(0x80u, 0x80u, 0x80u, 0xFFu));
				pDrawText(pTextThis(), x - 1, y + Salto + 4, "0", 77, 0, (PINT)1, 0);
				if (v13 == -1)
					SetTextColorByHDC(pTextThis(), pMakeColor(0xFFu/*0x80u*/, 0xFFu, 0xFFu, 0xFFu));
				else
					SetTextColorByHDC(pTextThis(), pMakeColor(0xFFu, 0xFFu, 0xFFu, 0xFFu));
				pDrawText(pTextThis(), x + 10, y + Salto + 4, QTPary.JCPartyDB[i].Name, 77, 0, (PINT)1, 0);
				gPartyBuffs.DrawPartyBuffs((double)x - 6, (double)y + Salto + 31, QTPary.JCPartyDB[i].Name);
			...
			else
			{
				...
				if (v13 == -1)
					...
					SetTextColorByHDC(pTextThis(), pMakeColor(0xFFu, 0xFFu, 0xFFu, 0xFFu));
				/*Flag*/
				//RenderBitmap(81528, x + 53, (double)(y + 4), Return2X(TransForX(9)), ReturnY(TransForY(9)), 0.0, 0.0, 1.0, 1.0, 1, 1, 0);
				RenderBitmap(81528, x-2.5, (double)(y + 4), Return2X(TransForX(9)), ReturnY(TransForY(9)), 0.0, 0.0, 1.0, 1.0, 1, 1, 0);
				pDrawText(pTextThis(), x + 10, y + Salto + 4, QTPary.JCPartyDB[0].Name, 77, 0, (PINT)1, 0);
				gPartyBuffs.DrawPartyBuffs((double)x - 6, (double)y + Salto + 31, QTPary.JCPartyDB[0].Name);
			}
			...
			if ((signed int)(unsigned __int8)QTPary.JCPartyDB[i].LifePercent <= 10)
				...
				v42 = 10;
			v39 = (double)v42 / 10.0 * 82.5;

			 pDrawGUI(81526, x - 5.2, y + Salto + 17, v39, 6.0);


			//ChangeButtonInfo((char*)(v43 + 172 * i + 32), x + Return2X(TransForX(63)), y + Salto + ReturnY(TransForY(3)), Return2X(TransForX(10)), ReturnY(TransForY(10)));
			/*if (!strcmp(QTPary.JCPartyDB[0].Name, (char*)(*(DWORD*)0x7BC4F04 + 56))
				|| !strcmp(QTPary.JCPartyDB[i].Name, (char*)(*(DWORD*)0x7BC4F04 + 56)))
			{
				if (CheckButtonPressed(x + Return2X(TransForX(65)), y + Salto + ReturnY(TransForY(4)), Return2X(TransForX(10)), ReturnY(TransForY(10)))) 
				{
					if (nInterface.IsWorkZone(x + Return2X(TransForX(65)), y + Salto + ReturnY(TransForY(4)), Return2X(TransForX(10)), ReturnY(TransForY(10)))) 
					{
						pDrawButton(81532, x + Return2X(TransForX(65)), y + Salto + ReturnY(TransForY(4)), Return2X(TransForX(10)), ReturnY(TransForY(10)), Return2X(TransForX(3)), ReturnY(TransForY(3)));
					}
					else 
					{
						pDrawButton(81530, x + Return2X(TransForX(65)), y + Salto + ReturnY(TransForY(4)), Return2X(TransForX(10)), ReturnY(TransForY(10)), Return2X(TransForX(3)), ReturnY(TransForY(3)));
					}
				}
				else
				{
	...
	*(DWORD *)(a1 + 924) = -1;
	...
	for (i = 0; i < QTPary.PartyMemberCount; ++i)
	{
			...
			|| !strcmp(QTPary.JCPartyDB[i].Name, (char *)(*(DWORD *)0x7BC4F04 + 56)))
		{
			...
			{
				if (pCheckMouseOver(*(DWORD*)(v9 + 24) + *(DWORD*)(v9 + 16) + 63 + JCResto + Fix_DisplayWinExt,
					v11 + *(DWORD*)(v9 + 28) + *(DWORD*)(v9 + 20) + 3,
					11, 11) && *(BYTE*)0x8793386)
				{
					ObjPartyDelect(i);
			...
			/*else if (gProtect.m_MainInfo.CustomInterfaceType == 2)
			{
				...
				if (pCheckMouseOver(*(DWORD*)(v9 + 24) + *(DWORD*)(v9 + 16) + 63 + JCResto + Fix_DisplayWinExt,
					v11 + *(DWORD*)(v9 + 28) + *(DWORD*)(v9 + 20) + 3 +v11,
					11, 11) && *(BYTE*)0x8793386)
				{
					ObjPartyDelect(i);
					return 1;
...
void PartyBar10::Load()
{
	...
	{
		SetCompleteHook(0xE9, 0x0084BDB0, &PrintfPartyBarEx700);
	}
